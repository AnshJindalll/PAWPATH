...
export function EnhancedHeroSection() {
  ...
  const handleSuggestionSelect = (suggestion: string) => {
    ...
    setShowSuggestions(false)
    setTimeout(() => {
      window.location.href = `/adopt?search=${encodeURIComponent(suggestion)}`
    }, 100)
  }

  const handleFilterClick = (filterId: string) => {
    setActiveFilter(filterId)
    // Navigate to adopt page with filter
    ...
    window.location.href = filterId === "all" ? "/adopt" : `/adopt?filter=${filterValue}`
  }
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden">
      ...
      <div className="container mx-auto px-4 relative z-10 text-center text-white">
        <div className="max-w-4xl mx-auto space-y-8">
          {/* Hero Title */}
          <ScrollAnimation direction="up" className="space-y-4">
          ...
          {/* Search Bar */}
          <ScrollAnimation direction="up" delay={200} className="max-w-2xl mx-auto space-y-6">
                ...
                <Button
                  ...
                  className={`rounded-full px-6 py-2 transition-all duration-300 hover:scale-105 ${
                    activeFilter === filter.id
                      ? "bg-white text-[var(--primary-olive)] hover:bg-white/90 shadow-lg"
